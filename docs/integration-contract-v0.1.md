...
Each environment publishes its explicit OIDC issuer at `OIDC_ISSUER` and its discovery at `${OIDC_ISSUER}/.well-known/openid-configuration`. Consumer configuration pins the full issuer and unique client ID; redirect URIs are exact HTTPS callback URLs (localhost HTTP only for development). Use Authorization Code + PKCE S256 and `state`, with `nonce` for ID tokens. Allowed base scopes: `openid profile email`. Do not infer identity from email.

Register clients through a controlled change, adding an exact redirect URI, audience mapper for `accesslobby-api`, and the client ID to API allowlist. Do not put secrets in Git. Public clients require PKCE; confidential server clients additionally authenticate at token exchange.
