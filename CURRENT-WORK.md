...
| Repo/bootstrap | MERGED; NETWORK CORRECTION IN REVIEW | PR #1 merged as `354a100` after passing checks; main CI run `36147843722` and publisher `36148016779` passed. API/web immutable digests were recorded from that publication. Dokploy v0.30.7 deprecates Isolated Deployment; the IAM gateway and declarative internal networks require a new PR, CI and matched image publication before staging deployment. Issues #2–#12 track follow-up. |
...
| Web handoff | BUILT, UNVERIFIED | PKCE, state, nonce, sealed cookie, HTTPS-only public origin and logout built; real browser smoke pending. |
...
| Staging host | PREPARED, NOT DEPLOYED | Owner installed Docker/Dokploy, created a private-tunnel admin account, verified DNS for three staging names and created an `AccessLobby / staging` Compose shell. The owner screenshot shows Isolated Deployment off and all six original services loaded in Networks; no deployment occurred. The new gateway service, Git source Save, TLS, protected secrets, realm import, routes, proxy headers and generated network preview require runtime verification/configuration. See `LIVE-ENVIRONMENT-FACTS.md` and issue #7. |
