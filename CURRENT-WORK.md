...
| Repo/bootstrap | DRAFT PR | Main bootstrap `966347b`; PR #1 security commit `c42f82c` passed CI run #28 (`verify`, `iam-smoke`). A prior owner-session GitHub UI inspection confirmed an active `Protect main` ruleset requiring PR and both checks, blocking force push/deletion; recheck at merge. The subsequent web-origin guard and test require final-head CI before merge. Issues #2–#12 track follow-up. |
